class pageActions {
  ...
  getData() {
    ...
    document.querySelector('.paining-container .img img').src = resdata.images[0].url;
    document.querySelector('.details .content .title').innerText = resdata.name;
    document.querySelector('.details .content .info').innerText = resdata.dynasties + '·' + resdata.factions;
    ...
    let otherList = resdata;
    otherList.images.forEach((item, index) => {
      otherListContainer.innerHTML += `
          <div class="item">
            ...
            <div class="content-box">
              <div class="detail">
                <h2>${item.name}</h2>
                <p>${otherList.dynasties + '·' + otherList.name}</p>
              </div>
            </div>
        </div>
        `;
